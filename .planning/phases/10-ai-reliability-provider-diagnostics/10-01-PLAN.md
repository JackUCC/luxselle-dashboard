...
<task type="auto">
  <name>Task 3: Health endpoint test_providers mode</name>
  <files>
    packages/server/src/server.ts
  </files>
  <action>
    Extend the /api/health route to support optional ?test_providers=1 query parameter.
...
7. 3 new tests pass; 0 regressions (npm test green)
</success_criteria>
