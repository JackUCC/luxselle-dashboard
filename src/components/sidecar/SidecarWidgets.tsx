...
export default function SidecarWidgets() {
  ...
  return (
    <div className="space-y-4 min-w-0">
      <div className="rounded-lg border border-lux-100 bg-lux-50/80 px-3 py-2">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-lux-500">Tools — Expand only what you need</h2>
        <ul className="mt-1.5 space-y-1 text-xs text-lux-600 list-disc list-inside">
      ...
      <div className="space-y-4">
        {WIDGETS.map((widget) => {
          ...
          return (
            <section
              key={widgetId}
              className={`rounded-lux-card border border-lux-200 bg-white transition-shadow duration-200 ${!isCollapsed ? 'shadow-sm' : ''}`}
            >
              <div className="flex items-start justify-between gap-2 px-3 py-2 border-l-2 border-l-lux-200 hover:bg-lux-50/50 transition-colors">
                ...
                <button
                  ...
                  className="inline-flex shrink-0 items-center justify-center gap-1 rounded-md border border-lux-200 px-2.5 py-1.5 min-h-[32px] text-xs font-medium text-lux-600 hover:bg-lux-100 hover:border-lux-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lux-gold/30 focus-visible:ring-offset-1 transition-colors"
                >
                  {isCollapsed ? (
                    <>
              ...
              >
                <div className="min-h-0 overflow-x-auto overflow-y-hidden">
                  <div className="border-t border-lux-100 p-2">
