        ...
        <>
            ...
            <div className="fixed right-0 top-0 h-full w-full max-w-xl bg-white shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-right duration-300 border-l border-lux-200">
                <div className="flex items-center justify-between px-6 py-4 border-b border-lux-100 bg-lux-50/50">
                    <h2 className="text-lg font-semibold text-lux-900">Import Inventory</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    {/* Instructions */}
                    <div className="rounded-lg bg-lux-200/80 p-5 border border-lux-300">
                    ...
                    {!result && (
                        <div
                            ...
                            className={`relative h-64 rounded-lux-card border-2 border-dashed transition-all cursor-pointer flex flex-col items-center justify-center gap-4 focus-visible:ring-2 focus-visible:ring-lux-gold/30 focus-visible:outline-none ${isDragOver
                                ? 'border-lux-600 bg-lux-200/80'
                                : 'border-lux-300 bg-lux-50 hover:bg-lux-100 hover:border-lux-400'
                        ...
                        >
                            ...
                            {isUploading ? (
                                <>
                                    ...
                                    <p className="text-sm font-medium text-lux-600">Importing products...</p>
                                </>
                            ) : (
                                <>
                                    <div className="h-16 w-16 rounded-full bg-white shadow-sm flex items-center justify-center border border-lux-100">
                                        <Upload className="h-8 w-8 text-lux-700" />
                                    </div>
                                    <div className="text-center">
                                        <p className="text-lg font-medium text-lux-900">Click to upload or drag and drop</p>
                                        <p className="text-sm text-lux-500 mt-1">Excel or CSV files only</p>
                                    </div>
                                </>
                            )}
                        </div>
                    ...
                    {result && (
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            {hasMissingInfo && (
                                <div className="rounded-lg border border-amber-200 bg-amber-50 p-5">
                                    ...
                                    <button
                                        ...
                                        className="mt-2 text-sm font-medium text-amber-700 hover:text-amber-900 underline focus-visible:ring-2 focus-visible:ring-lux-gold/30 focus-visible:outline-none"
                                    >
                                        View in list
                                    </button>
                                </div>
                                ...
                                <div className="rounded-lg border border-red-200 bg-white overflow-hidden shadow-sm">
                                    ...
                                    <div className="max-h-60 overflow-y-auto">
                                        <table className="min-w-full divide-y divide-lux-100">
                                            <thead className="bg-lux-50">
                                                <tr>
                                                    <th className="px-4 py-2 text-left text-xs font-medium text-lux-500 uppercase">Row</th>
                                                    <th className="px-4 py-2 text-left text-xs font-medium text-lux-500 uppercase">Error</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-lux-100">
                                                {result.errorDetails.map((err, i) => (
                                                    <tr key={i}>
                                                        <td className="px-4 py-2 text-sm text-lux-500 font-mono">{err.row}</td>
                            ...
                            <button
                                ...
                                className="w-full lux-btn-secondary focus-visible:ring-2 focus-visible:ring-lux-gold/30 focus-visible:outline-none"
                            >
                                Upload Another File
                            </button>
                        </div>
